"""
Snapshot Capture for Temporal Archival
...
Use cases:
- Temporal diff analysis (compare snapshots over time)
...
class CapturedSnapshot:
    """A captured snapshot with all requested formats."""
...
class SnapshotComparison:
    ...
    snapshot_a: CapturedSnapshot
    snapshot_b: CapturedSnapshot
...
class SnapshotCapturer:
    ...
    Example:
        ...
        snapshot = await capturer.capture(
            "https://example.com",
    ...
    async def __aenter__(self) -> "SnapshotCapturer":
    ...
    async def capture(
        ...
        Returns:
            CapturedSnapshot with requested formats
        """
    ...
    async def capture_many(
        ...
        Returns:
            List of CapturedSnapshot objects
        """
        import asyncio

        semaphore = asyncio.Semaphore(max_concurrent)

        async def capture_with_semaphore(url: str) -> CapturedSnapshot | None:
...
class TemporalArchive:
    """
    Manages temporal snapshots for diff analysis.

    Example:
        archive = TemporalArchive(Path("./archives/topic_x"))

        # Capture initial snapshot
        await archive.capture_snapshot("https://example.com/article")
    ...
    async def _get_capturer(self) -> SnapshotCapturer:
        if self._capturer is None:
            self._capturer = SnapshotCapturer()
            await self._capturer._get_crawler()
    ...
    def get_latest_snapshot(self, url: str) -> CapturedSnapshot | None:
    ...
    async def capture_snapshot(
        ...
        """Capture and archive a snapshot."""
        config = config or SnapshotConfig(
            capture_screenshot=True,
            capture_html=True,
            capture_mhtml=True,
        ...
        capturer = await self._get_capturer()

        # Use URL-based prefix
        prefix = self._url_to_prefix(url)
        snapshot = await capturer.capture(url, config)
    ...
    async def capture_and_compare(
        ...
        Returns:
            SnapshotComparison with diff information
        """
        # Get previous snapshot
        previous = self.get_latest_snapshot(url)

        # Capture new snapshot
        current = await self.capture_snapshot(url, config)
...
async def capture_snapshot(
    ...
    """
    Quick helper to capture a snapshot.
    ...
    async with SnapshotCapturer() as capturer:
        return await capturer.capture(url, config)
