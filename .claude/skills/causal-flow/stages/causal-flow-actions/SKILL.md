...
stage: 5-actions
action_id: engineering-sdk-white-label
...
# Action: Build White-Label SDK Generation

## Objective
Create SDK generation endpoint that outputs clean SDK without GlamYouUp branding,
...
## Deliverables
1. **POST /sdk/generate endpoint with white-label flag**
   - Acceptance criteria: Returns SDK bundle without GlamYouUp branding when white_label=true
...
**Implementation steps:**
1. Add white_label flag to /sdk/generate endpoint
...
4. Create client namespace deployment script (Terraform + Helm)
...
- [x] ElsaAI brand assets collected (logo, colors) - For testing

**Dependent on this action:**
- legal-contract-review (can proceed in parallel, but deployment needs contract)
- sales-collateral-enterprise (needs screenshots of white-label SDK)
...
## Success Criteria
**Done when:**
- [x] ElsaAI-branded SDK generated successfully
...
### Tasks
- [x] Design white-label endpoint API
...
**2025-11-18:** ElsaAI test SDK generated successfully. Generation time: 42 minutes. Well under 1 hour target.

**2025-11-20:** Security review passed. Verified no GlamYouUp branding leaks, namespace isolation strong, monitoring comprehensive.

---

## Linked Documents
- Decision: threads/business/enterprise-white-label/4-decision.md
- Related actions:
...
**Examples:**
- `engineering-sdk-white-label.md`
...
**Example:**
```
engineering-sdk-white-label (week 1-2)
  ↓
sales-collateral-enterprise (week 3)
  (needs screenshots from white-label SDK)
```
...
**Example:**
```
Action: sales-close-elsa (blocked)
...
❌ Single action: "Build white-label and finalize contracts"
✓ Separate actions:
- engineering-sdk-white-label.md
- legal-contract-review.md
...
❌ "Improve SDK"
✓ "Add white-label flag to /sdk/generate endpoint that returns SDK without branding"
...
✓ "POST /sdk/generate endpoint with white_label flag that removes GlamYouUp branding"
