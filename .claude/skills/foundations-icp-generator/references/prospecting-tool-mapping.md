...
Example ICP Mapping:
ICP: "{Target industry} e-commerce"
→ Sales Nav: Select "{Industry Category}" + "Internet"
```
...
Example ICP Mapping:
Economic Buyer: "VP {Department}, Director {Department}"
→ Sales Nav:
  Title: "VP {Department}" OR "Director {Department}" OR "VP {Related Function}" OR "Director {Related Function}"
```
...
Example ICP Mapping:
Problem Signal: "Hiring for {problem-related role} (indicates {problem indicator})"
→ Sales Nav:
  - Posted jobs in last 90 days
  - Job title keywords: "{Role Keyword}" OR "{Related Role}" OR "{Department}"
```
...
```
Search: https://builtwith.com/websites-using/{Platform}
...
Example ICP Mapping:
ICP: "{Platform} stores"
→ BuiltWith: Search for "{Platform}" technology
```

**Payment Processors**
```
Search: https://builtwith.com/websites-using/{Payment Processor}
...
→ BuiltWith: Filter for "{Modern Processor}" or "{Alternative Processor}"
```
...
Technologies tracked:
  ...
  - {Analytics Tool B}
  - {Analytics Tool C}
  - {Email Marketing Tool} (email marketing)
  - {Marketing Automation Tool A}
  - {Marketing Automation Tool B}
...
Some verticals can be detected via technology:
  - {Vertical A}: {Related Tool A}, {Related Tool B}
  - {Vertical B}: {Payment Tool} + {Support Tool}
  - {Vertical C}: {CMS Platform} + {Monetization Tool}
  - {Vertical D}: {POS System}, {Delivery Integration}

Example:
ICP: "{Target vertical} e-commerce"
→ BuiltWith:
  - {Platform} + {Vertical-Specific Tool A}
  - Or: {Platform} + {Vertical-Specific Tool B}
```
...
Example:
ICP: "Modern {Platform} stores (not using old tracking)"
→ BuiltWith:
  - {Platform} AND {Modern Analytics Tool}
  - NOT {Legacy Analytics Tool}
```
...
Example:
ICP: "E-commerce > {Target Category}"
→ SimilarWeb: Filter by category "Shopping > {Target Category}"
```
...
**FAQ/About Pages**
```
Query: site:example.com "{problem keyword}" OR "{related policy}"

Example ICP Mapping:
Problem Signal: "Mentions {problem indicator} on website"
→ Google: site:example.com ("{problem severity}" OR "{related policy}" OR "{problem FAQ}")
```

**Job Postings**
```
Query: site:lever.co OR site:greenhouse.io "company-name" "{Problem-Related Role}"

Example ICP Mapping:
Signal: "Hiring for {problem-related role} ({problem indicator})"
→ Google:
  - site:lever.co "company-name" ("{Role Keyword}" OR "{Related Role}")
  - site:greenhouse.io "company-name" ("{Role Keyword}" OR "{Related Role}")
```

**Technology Stack**
```
Query: site:example.com "powered by {Platform}"

Example:
Verify Platform: Is this a {Platform} store?
→ Google: site:example.com ("powered by {Platform}" OR "{platform-domain}.com")
```

**Review Mentions**
```
Query: site:trustpilot.com OR site:reviews.io "company-name" "{problem keyword}"

Example ICP Mapping:
Problem Signal: "Customer complaints about {problem area}"
→ Google:
  - site:trustpilot.com "brand-name" ("{problem keyword}" OR "{related complaint}" OR "{symptom A}" OR "{symptom B}")
```
...
Example:
ICP: "E-commerce {industry}"
→ Apollo:
  - Industry: Retail, {Industry Category}
...
Example:
ICP: "{Platform} stores using {Marketing Tool}"
→ Apollo:
  - Technologies: {Platform}, {Marketing Tool}
```
...
Example:
ICP: "{Platform} {premium tier} stores (enterprise)"
→ ZoomInfo: Installed Technologies: "{Platform} {Premium Tier}"
```
...
### Workflow 1: {Platform} Store with Problem Signals

**Step 1 - BuiltWith**: Find {Platform} stores in target geography
```
Technology: {Platform}
Country: United States
Traffic: Medium to High
→ Export list of 500 {Platform} sites
```
...
For each site, search:
  site:[domain] ("{problem keyword}" OR "{related policy}")
  site:[domain] ("{problem indicator}" OR "{related guide}")
...
Filter:
  - Title: "VP {Department}" OR "Director {Department}" OR "Founder"
  - Seniority: Director, VP, CXO
...
Import company list
Filter:
  - Technologies: {CRM System}, {Marketing Tool} (suggests sales maturity)
...
For each company, find:
  - Economic buyer: VP {Department}, C{X}O
  - Technical buyer: VP {Technical Function}, CTO
  - Champion: {Department} Ops Manager
...
prospecting_queries:
  builtwith:
    - query: "{Platform} stores in United States"
      filters:
        - "Technology: {Platform}"
        - "Country: United States"
        - "Traffic: Medium or High"
      url: "https://builtwith.com/{platform}"

  linkedin_sales_navigator:
    - company_filters:
        industry: ["{Industry Category A}", "{Industry Category B}"]
        company_size: "51-200 employees"
        geography: "United States"
      contact_filters:
        title: ["VP {Department}", "Director {Department}", "Founder"]
        seniority: ["Director", "VP", "CXO"]

  similarweb:
    - filters:
        monthly_visits: "10,000 - 100,000"
        category: "Shopping > {Target Category}"
        geography: ">70% United States traffic"

  google_search:
    - query: 'site:{domain} ("{problem keyword}" OR "{related policy}" OR "{problem indicator}")'
