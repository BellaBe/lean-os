...
Search: `site:example.com "{problem keyword}" OR "{related policy}"`

**Positive signals**:
```
✓ FAQ: "What's your {policy} policy?"
✓ FAQ: "Why do {problem symptoms} happen?"
✓ Policy page: "We offer {mitigation} for {problem area}"
✓ Product pages: "See our {solution guide}" (prominent placement)
✓ Blog: "How to {avoid problem}"
```
...
```
site:example.com ("{policy keyword}" OR "{mitigation}" OR "{problem indicator}" OR "{solution guide}")
```
...
**Positive signals**:
```
✓ {Solution tool} on every {relevant page} (prominent)
✓ "{Problem warning}" notices
✓ {Generous policy} (suggests they expect {problem})
✓ "{Mitigation}" prominently marketed (cost of doing business)
✓ Video guides on "How to {avoid problem}"
```
...
**Positive signals**:
```
✓ 10+ reviews mentioning "{problem area}"
✓ 3-star reviews (not terrible, but frustrated with specific issue)
✓ Positive reviews that mention "{mitigation policy}" (implies problem is common)
```
...
Search LinkedIn: `"company name" AND ("{problem-related role}" OR "{related specialist}" OR "{related function}")`

**Positive signals**:
```
✓ "{Problem-Related Role}" role posted
✓ "{Department} - {Problem Specialist}"
✓ "{Related Function} Manager"
✓ Job description mentions "high volume {problem processing}"
```
